...
Directory-scoped instructions live under `.github/instructions/*.instructions.md` (an `applyTo` glob targets each subtree): `qml.instructions.md`, `src.instructions.md`, `tests.instructions.md`.
...
Agents launched with `isolation: "worktree"` get a worktree at `D:\qvim\.claude\worktrees\agent-<id>` but **do not get path translation**. The edit/create tools take absolute paths verbatim.
