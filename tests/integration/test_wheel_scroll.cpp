...
private:
    ...
    int leftcol() {
        m_conn->execLua(QStringLiteral("vim.g.qvim_test_leftcol = vim.fn.winsaveview().leftcol"));
        std::optional<QVariant> got;
        bool done = false;
        m_conn->getVar(QStringLiteral("qvim_test_leftcol"), [&](std::optional<QVariant> v) {
            got = v;
            done = true;
        });
    ...
    void sendWheel(int dx, int dy, Qt::KeyboardModifiers mods = Qt::NoModifier) {
        ...
        QWheelEvent ev(scenePos, globalPos, QPoint(0, 0), QPoint(dx, dy), Qt::NoButton, mods,
...
private slots:
    ...
    void horizontalWheelPansView() {
        ...
        QVERIFY2(afterRight > afterLeft,
                 qPrintable(QStringLiteral(
                                "horizontal wheel (angleDelta.x=-120) did not pan the view right: "
                                "leftcol %1 -> %2 (expected an increase)")
                                .arg(afterLeft)
                                .arg(afterRight)));
