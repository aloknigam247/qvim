...
private slots:
    ...
    void leftDragEntersVisualMode() {
        ...
        QVERIFY2(waitUntil(
                     [&] {
            return conn.modeInfo()->currentName().contains(QStringLiteral("visual"),
                                                           Qt::CaseInsensitive);
        }, 3000),
        ...
        for(int i = 0; i < 3; ++i) waitForFlush(&conn, 500);
        conn.command(
            QStringLiteral("call setline(2, getpos(\"'<\")[2] . ',' . getpos(\"'>\")[2])"));
        for(int i = 0; i < 3; ++i) waitForFlush(&conn, 500);
        ...
        QVERIFY(g);
        QString row1;
        for(int c = 0; c < g->gridCols(1); ++c) { row1 += g->cell(1, 1, c).text; }
        ...
        QVERIFY2(match.hasMatch(), qPrintable(QStringLiteral("Could not parse selection range "
                                                             "from row 1 ('%1')")
                                                  .arg(row1)));
