...
private slots:
    ...
    void holdingJStaysSnappy() {
        ...
        QVERIFY(waitUntil([&] { return conn.grid()->cols() == 200 && conn.grid()->rows() >= 58; },
                          5000));
        ...
        QElapsedTimer t;
        t.start();
        for(int i = 0; i < kKeys; ++i) { QTest::keyClick(window, Qt::Key_J); }
        ...
        qDebug("holdingJ: %d keys in %lld ms -> %.2f ms/key", kKeys, totalMs,
               static_cast<double>(totalMs) / kKeys);
