...
app (client). It is intentionally minimal for this milestone; later slices extend
...
3. The server then streams live events. In this milestone the client always sends `lastSeq: 0`
...
| `message`       | `seq`, `id`, `role`, `text`             | an atomic message block                  |
...
streamed chunks to the right author. The current backend (the copilot bridge) delivers each block
atomically as a `message` frame; the streamed `begin`/`delta`*/`end` shape is reserved for a future
incremental backend and is a no-op for clients until one emits it.

```json
{"type":"hello","protocol":1,"sessionId":"7f3c…"}
{"seq":1,"type":"message","id":"u1","role":"user","text":"hi"}
{"seq":2,"type":"message","id":"a1","role":"assistant","text":"hello there"}
```
