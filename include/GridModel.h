...
public:
    ...
    void clear()                                                                      { clear(1); }
    void applyLine(int row, int colStart, const msgpack::object& cellsArr)            { applyLine(1, row, colStart, cellsArr); }
    void scroll(int top, int bot, int left, int right, int rows)                      { scroll(1, top, bot, left, right, rows); }
    void setCursor(int row, int col)                                                  { setCursor(1, row, col); }

    // --- Active-grid passthrough accessors ---
    int cols()       const { const auto* s = surface(m_active); return s ? s->cols      : 0; }
    int rows()       const { const auto* s = surface(m_active); return s ? s->rows      : 0; }
    int cursorRow()  const { const auto* s = surface(m_active); return s ? s->cursorRow : 0; }
    int cursorCol()  const { const auto* s = surface(m_active); return s ? s->cursorCol : 0; }
