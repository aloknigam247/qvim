...
// Per-grid surface state. The global grid (id=1) always exists; additional grid ids keep
// their own cells and dirty state for redraw-event compatibility.
struct GridSurface {
    int cols = 0;
    int rows = 0;
    int cursorRow = 0;
...
class GridModel : public QObject {
    ...
    // Active-grid passthrough — for back-compat with the single-grid renderer
