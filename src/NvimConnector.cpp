        ...
        if(res) {
            ...
            m_rpc->request(QStringLiteral("nvim_get_option_value"),
                ...
                if(getRes) {
                    ...
                    if(o.type == msgpack::type::STR && o.via.str.size > 0) { empty = false; }
                }
                if(empty) {
...
void NvimConnector::input(const QString &keys) {
    m_rpc->notify(QStringLiteral("nvim_input"), [&keys](msgpack::packer<msgpack::sbuffer> &pk) {
        pk.pack_array(1);
...
void NvimConnector::paste(const QString &text) {
    m_rpc->request(QStringLiteral("nvim_paste"), [&text](msgpack::packer<msgpack::sbuffer> &pk) {
        pk.pack_array(3);
...
void NvimConnector::command(const QString &cmd) {
    m_rpc->request(QStringLiteral("nvim_command"), [&cmd](msgpack::packer<msgpack::sbuffer> &pk) {
        pk.pack_array(1);
...
void NvimConnector::loadStdinIntoBuffer(const QByteArray &bytes) {
    ...
    m_rpc->notify(QStringLiteral("nvim_command"), [](msgpack::packer<msgpack::sbuffer> &pk) {
        pk.pack_array(1);
...
void NvimConnector::execLua(const QString &code) {
    m_rpc->request(QStringLiteral("nvim_exec_lua"), [&code](msgpack::packer<msgpack::sbuffer> &pk) {
        pk.pack_array(2);
...
void NvimConnector::onOptionSet(const QString &name, const QVariant &value) {
    ...
    if(name.startsWith(QStringLiteral("ext_"))) { return; }
...
void NvimConnector::dispatchEvent(const std::string &name, const msgpack::object &evt) {
    ...
    if(name == "grid_cursor_goto") {
        // [grid, row, col]
        if(a.size >= 3)
            m_grid->setCursor(static_cast<int>(asInt(a.ptr[0])), static_cast<int>(asInt(a.ptr[1])),
                              static_cast<int>(asInt(a.ptr[2])));
        return;
    }
    if(name == "grid_line") {
        ...
        if(a.size >= 4)
            m_grid->applyLine(static_cast<int>(asInt(a.ptr[0])), static_cast<int>(asInt(a.ptr[1])),
                              static_cast<int>(asInt(a.ptr[2])), a.ptr[3]);
        return;
    }
    if(name == "grid_scroll") {
        // [grid, top, bot, left, right, rows, cols]
        if(a.size >= 7) {
            m_grid->scroll(static_cast<int>(asInt(a.ptr[0])), static_cast<int>(asInt(a.ptr[1])),
        ...
        if(a.size >= 2) {
            ...
            switch(v.type) {
                case msgpack::type::BOOLEAN:
                    ...
                    break;
                case msgpack::type::POSITIVE_INTEGER:
                    qv = static_cast<qlonglong>(v.via.u64);
                    break;
                case msgpack::type::NEGATIVE_INTEGER:
                    qv = static_cast<qlonglong>(v.via.i64);
                    break;
                case msgpack::type::STR:
                    qv = QString::fromUtf8(v.via.str.ptr, v.via.str.size);
                    break;
                default:
                    break;
            }
