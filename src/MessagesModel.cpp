...
int MessagesModel::dominantAttr(const msgpack::object& content) {
    ...
    for (uint32_t i = 0; i < arr.size; ++i) {
        ...
        const int len = (textObj.type == msgpack::type::STR)
                            ? static_cast<int>(textObj.via.str.size)
                            : 0;
        if (len > bestLen) {
            bestLen = len;
            best = attrId;
...
void MessagesModel::msgShow(const msgpack::object& kindObj,
                            const msgpack::object& content,
                            bool replaceLast) {
...
QVariant MessagesModel::data(const QModelIndex& index, int role) const {
    ...
    switch (role) {
        ...
        case AttrIdRole: return it.attrId;
        default:         return {};
    }
}
